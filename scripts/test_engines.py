...
def test_engines():
    ...
    # "The Control Throttle must be 1, you modulate each engine via their thrust_limit. 
    # The engines must have independent_throttle set to true."
    print("Setting main vessel throttle to 1.0")
    vessel.control.throttle = 1.0

    tagged_parts = vessel.parts.with_tag("AegisEngine")
    if not tagged_parts:
        print("No parts tagged 'AegisEngine' found. Falling back to all engines.")
        engines = [part.engine for part in vessel.parts.engines if part.engine is not None]
    else:
        engines = [part.engine for part in tagged_parts if part.engine is not None]

    print(f"Found {len(engines)} engines to test.")

    # Configure each engine
    for i, engine in enumerate(engines):
        print(f"Configuring Engine {i}: independent_throttle=True, thrust_limit=0.0")
        engine.active = True
    ...
    try:
        for i, engine in enumerate(engines):
            ...
            time.sleep(0.5)
            print(f"   -> active: {engine.active}, has_fuel: {engine.has_fuel}, current_thrust: {engine.thrust:.2f} N")
            time.sleep(0.5)
