...
def test_detect_fault():
    ...
    for _ in range(49):
        assert not fdi.detect_fault(expected, measured_fault)
    assert fdi.detect_fault(expected, measured_fault)

def test_isolate_fault():
    fdi = FaultDetectionIsolation(threshold=0.5)
    
    e0 = Engine(index=0, position=np.array([ 1.0, 0.0,  0.0]), thrust_direction=np.array([ 0.1, 0.99, 0.0]), max_thrust=100.0, part=None)
    e1 = Engine(index=1, position=np.array([-1.0, 0.0,  0.0]), thrust_direction=np.array([-0.1, 0.99, 0.0]), max_thrust=100.0, part=None)
    e2 = Engine(index=2, position=np.array([ 0.0, 0.0,  1.0]), thrust_direction=np.array([ 0.0, 0.99, 0.1]), max_thrust=100.0, part=None)
    e3 = Engine(index=3, position=np.array([ 0.0, 0.0, -1.0]), thrust_direction=np.array([ 0.0, 0.99,-0.1]), max_thrust=100.0, part=None)
