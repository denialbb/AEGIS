...
def test_sensor_models_noise_statistics():
    ...
    def add_stream_side_effect(func, obj, attr):
        ...
        for _ in range(n_samples):
            ...
            result = sensors.poll()
            noisy_alt, noisy_accel, attitude, mass = (
                result[0],
                result[1],
                result[2],
                result[3],
            ...
            # Verify attitude is a valid unit quaternion (Mahony filter estimate)
            assert len(attitude) == 4
            # Should be unit quaternion (norm close to 1)
            assert abs(np.linalg.norm(attitude) - 1.0) < 0.01
        assert mass == 5000.0
        ...
        assert np.isclose(
            np.std(accel_samples[:, 0]), config.SIGMA_ACCEL, rtol=0.05
        )
        assert np.isclose(
            np.std(accel_samples[:, 1]), config.SIGMA_ACCEL, rtol=0.05
        )
        assert np.isclose(
            np.std(accel_samples[:, 2]), config.SIGMA_ACCEL, rtol=0.05
        )
