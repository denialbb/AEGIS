...
class ControlAllocator:
    ...
    def allocate(
        ...
        if N == 0:
            logger.warning(
                "No active engines available for control allocation."
            )
                ...
                if dot_prod < 0:
                    ...
                    if throttle > 1.0:
                        ...
                        if engine.index not in self._saturated_engines:
                            logger.warning(
                                f"Engine {engine.index} thrust saturated (requested: {f_mag:.2f}, max: {engine.max_thrust:.2f})"
                            )
                            self._saturated_engines.add(engine.index)
