...
> `DECISIONS.md`, `OPEN_ISSUES.md`, current `estimator.py` / `controller.py` /
...
1. Merge two conceptually different observers (a Kalman filter for kinematic
   state, and an ESO for control-law disturbance estimation) into one module,
   which re-opens the module-boundary debate ADR-003/ADR-014 already settled.
...
|---|---|---|---|
| 1.1 EKF | Cornman & Mei (EKF) | Q/R tuning-by-measured-variance methodology (Section 3 of the paper) is directly reusable for ISS-003. | The paper's "EKF" is a 6-state Euler-angle filter for head orientation — it has **no analog to AEGIS's position/velocity state**, and its equations (as transcribed in the plan) are linear, not extended. The paper also reports the EKF cost **2.87× the loop time** of a simpler filter — a real budget concern at 50Hz. |
| 1.2 ESO | Leblebicioglu et al. (gimbal NN-ADRC) | ESO equations and `fal()` transcribed correctly; β/α/δ/b0 parameter set matches. | The ESO is a **single-channel** observer driven by `u` (control input) and `y` (plant output) for *that controller's* model — it is not a sensor-fusion filter and doesn't belong next to the Kalman filter (see §3.1). |
...
"measurement noise" in the Kalman sense; it's part of the ADRC loop.
...
- `y` = the *already-filtered* KF position/velocity output (reuse, don't
  duplicate, the existing Kalman filter's z1/z2-equivalent — this also avoids
...
This keeps `estimator.py` exactly as scoped in ADR-007/ADR-014, and keeps the
...
The equations transcribed in plan section 1.1 are the **standard linear KF**
predict/update equations (`A`, `H`, `Q`, `R`, `P`, `K` — no Jacobians, no
nonlinear `f()`/`h()`). Calling this an "EKF" only makes sense if the plan
intends to fold something nonlinear — most plausibly **attitude (quaternion)**
— into the filter state. If that's the intent, it directly reopens ADR-014,
which deliberately chose the small-angle approximation specifically *to avoid*
needing an EKF and its Jacobians. If that's *not* the intent, then "EKF" here
is a misnomer for the existing linear KF and section 1.1 adds nothing new.

There's a second ambiguity buried in "process noisy telemetry into clean
position, velocity, **and acceleration** estimates": this implies acceleration
becomes part of the state vector (a 9-state constant-acceleration model). But
ADR-007 ("Option A") already uses the noisy accelerometer reading as the
filter's **control input `u`**, not as a state to be estimated. You can't
cleanly do both — if accel is a filter *state*, the control input becomes
jerk (unmodeled, folded into Q), and the existing `B` matrix in `predict()`
(estimator.py lines ~55-60) would need to be redesigned from scratch.

**This needs an explicit decision, not an implicit one.** I'd frame it as a
new ADR with three options: (A) keep the current 6-state linear KF unchanged
and source filtered acceleration for the NN/ESO from elsewhere (e.g., the
ESO's own `z2` derivative, or the existing `expected_accel`/`noisy_accel_body`
directly); (B) extend to a 9-state constant-acceleration KF (still linear,
no EKF needed, but a genuinely different `F`/`B`/`Q` design than ADR-007's);
(C) fold attitude into the state and accept the EKF + Jacobian + ADR-014
reopening. My read of the literature is that **(A) is sufficient** — the NN's
acceleration input can come from the existing accelerometer + attitude
rotation already computed in `predict()` (the `kinematic_accel_world` local
variable), without touching the filter at all.
...
both depend on the Kalman filter's output quality. ISS-003 (Q/R are still
...
| **6 (deferred)** | Translation-axis ADRC/NN (full 6-DOF), and/or EKF + attitude-in-state-vector (§3.2 option C) if Phase 1-5 experience shows the small-angle approximation (ADR-014) is actually limiting. | Only revisit ADR-014 if there's evidence it's needed — don't reopen a settled, well-reasoned decision speculatively. | — |
...
- **ADR-027 — ESO/ADRC ownership and Guidance/Estimator boundary.** Formalizes
  §3.1: ESO lives in Guidance, fed by existing KF output; State Estimator
  remains scoped to ADR-007/014.
